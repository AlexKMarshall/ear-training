...
**Exercise orchestrator** (`mountExercise`, `mountMixedLesson`):
Lesson-screen mount wired by the **page mount**: owns **exercise screen state**, response-mode adapters (sing recording vs select choices), Solid presentation wiring, and **attempt scored** persistence. Definitions supply behaviour; the orchestrator does not branch on practice mode for persistence. **Homogeneous lessons** (one **practice mode** for the whole run) use `mountExercise`. **Mixed-type lessons** (each slot carries its own **practice mode** and **exercise definition** — e.g. **targeted practice**) use `mountMixedLesson`, which rebuilds orchestration per slot and may swap **response mode** mid-lesson; in-lesson advance auto-plays the next reference (same rhythm as homogeneous advance), while the first slot still waits for Play.
_Avoid_: Per-mode mount functions in the registry, mountSingTest / mountIdentifyTest (legacy), bespoke orchestration forks in page mounts

...
| TD-017 | P2 | UI architecture | Exercise UIs built as **imperative DOM strings** inside mount functions rather than component-based presentation. Home guided path uses **Solid JSX**; exercise mounts still string-built. | Harder to test presentation separately; large mount diffs when refactoring exercise screen state. | Incremental **component-based presentation** inside remaining **page mounts** (see `CONTEXT.md` — *Presentation implementation*, *Page mount*). **Lesson run** is already extracted; not blocked on TD-002. |

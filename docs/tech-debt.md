...
| TD-014 | P1 | Routing / pages | Each **practice mode** has its own HTML entry, Vite build input, and URL (`/interval-melodic-sing/`, …). **Mixed-type lessons** (targeted practice; later guided path?) need one mount to swap definitions mid-run — the per-mode MPA layout fights that. | **Targeted practice** ships a second orchestration path beside existing mode pages; every new exercise type adds another standalone route. | Unify on session-centric routes (e.g. `/lesson/` with session kind + plan in URL or storage); registry drives behaviour without one page per mode. Targeted practice is the forcing function; migrate guided replay after. Overlaps **TD-010** (per-exercise page boilerplate). |

...
|----|----------|------|---------|----------------|---------------------|
| TD-001 | P0 | Exercise model | Parallel **`SingTestConfig`** / **`IdentifyTestConfig`** and separate **`mountSingTest`** / **`mountIdentifyTest`** paths. Registry lists `responseMode` but there is no unified **`ExerciseDefinition`** with shared `prepareExercise`, `playReference`, `score(response)`. | Blocks Phase 1–2 goal of one practice-mode abstraction; every new practice mode duplicates mount wiring. | Introduce `ExerciseDefinition` incrementally: shared question/score types, single lesson orchestrator, thin per-exercise config. See [Architectural direction](roadmap.md#architectural-direction). |

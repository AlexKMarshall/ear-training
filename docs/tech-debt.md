...
| TD-004 | P0 | Curriculum | Two overlapping models: **`CURRICULUM_LEVELS`** / **`CURRICULUM_PATH`** (exercise-level home UI) vs **`CURRICULUM_STEPS`** (exercise + tier). Step unlock exists (`isStepUnlocked`) but **`getUnlockRequirement`** and home copy still describe **exercise-level** predecessors. | Content-tier unlock and cross-mode sequencing ([Progressive difficulty model](roadmap.md#progressive-difficulty-model)) need one authoritative step graph. | Make `CURRICULUM_STEPS` the source of truth for home, Continue, and guards; demote levels to display grouping only; update unlock copy to tier-aware labels. |
...
| TD-008 | P1 | Curriculum sequencing | Unlock follows flat **`CURRICULUM_STEPS`** order; **cross-mode sequencing** (melodic → harmonic → ID at same tier before next tier) is documented but not enforced. **`session-step.ts`** pins harmonic interval exercises to **`interval-2a`** regardless of melodic tier progress. | Phase 1 tier ladder and pedagogical ordering cannot be expressed without ad-hoc rules scattered in session-step. | Model steps with explicit predecessor links (mode + tier); drive both unlock and `getSessionStepForExercise` from that graph. |
...
*(No items closed yet. When merging a debt PR, move the row here with PR link and date, or delete the row if the registry stays short.)*

...
| TD-017 | P2 | UI architecture | Exercise UIs built as **imperative DOM strings** inside mount functions rather than component-based presentation. | Harder to test presentation separately; large mount diffs when refactoring exercise screen state. | Incremental **component-based presentation** inside existing **page mounts** (see `CONTEXT.md` — *Presentation implementation*, *Page mount*). **Lesson run** is already extracted; not blocked on TD-002. Land as two PRs: (1) Solid/Vite/Vitest plumbing + trivial browser smoke, (2) home guided path JSX; further mounts scheduled separately. |

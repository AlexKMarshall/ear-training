...
| **Testing** | **Partial** — 12 Node unit test files (domain, curriculum, history stats); **GitHub Actions CI** ([`ci.yml`](../.github/workflows/ci.yml): `npm test` + build on every PR). **No** automated UI/orchestration tests yet. **Next:** [T0](testing-roadmap.md#phase-t0---foundation-tooling--first-ports) — Vitest browser mode + injectable ports. |
...
- **Today:** Vitest in Node covers scoring, generation, unlock rules, and stats — not mount flows, rounds in the DOM, or history writes from UI. **CI** runs `npm test` + `npm run build` on every PR and `main`.
- **Direction:** [Vitest browser mode](https://vitest.dev/guide/browser/) (real Chromium); **no** jsdom/happy-dom UI tests; **no** mocking `smplr` / `pitchy` or audio modules — inject **ports** at `mount*` boundaries (`HistoryPort`, `RecordingPort`, etc.) alongside existing config hooks (`prepareQuestion`, `playReference`).
- **Phases (testing):** ~~**T-CI**~~ **Done** — GitHub Actions on every PR → **T0 next** (browser tooling + history port + home/locked guards) → T1 identify orchestration → T2 sing with fake recording → T3 smoke per new registry exercise. Manual QA remains for mic, permissions, and timbre.

Run alongside product phases below; land **T0–T1** before large Phase 2 UI churn.
...
| Automated UI regression | Todo | [Testing roadmap T0–T2](testing-roadmap.md#phased-plan) — browser orchestration tests; domain tests already cover stats/unlock math. |
| CI on every PR | **Done** | [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) — `npm test` + `npm run build`; see [T-CI](testing-roadmap.md#phase-t-ci---github-actions-baseline) |
...
| Regression safety as features grow | **CI baseline done**; [Testing T0–T3](testing-roadmap.md#phased-plan) for UI ports + browser tests | Manual full-path QA does not scale past ~6 exercises |
...
0. ~~[T-CI](testing-roadmap.md#phase-t-ci---github-actions-baseline)~~ — **Done** — GitHub Actions: `npm test` + `npm run build` on PRs and `main`
1. **[T0](testing-roadmap.md#phase-t0---foundation-tooling--first-ports) — next** — Vitest browser project, `HistoryPort`, home + locked-page tests *(before or with next UI-heavy PR)*
2. [T1](testing-roadmap.md#phase-t1---identify-exercise-orchestration) — identify round + `saveAttempt` in browser
3. [T2](testing-roadmap.md#phase-t2---sing-exercise-orchestration) — fake `RecordingPort`, sing pass/fail paths
...
- **Testability at UI boundaries** — thread `HistoryPort` / `AudioPort` / `RecordingPort` into mount functions; production defaults in page entrypoints only. Enables [browser orchestration tests](testing-roadmap.md#dependency-ports-enabler) without mocking vendor audio libs.
